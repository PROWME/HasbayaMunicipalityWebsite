"use client";

import BackToRequests from "@/components/BackToRequests";
import BuildingStatusSelector from "@/components/BuildingStatusSelector";
import ClearanceInfoSection from "@/components/ClearanceInfoSection";
import ClearanceRequestSection from "@/components/ClearanceRequestSection";
import DocumentUploader from "@/components/DocumentUploader";
import InfoForm from "@/components/InfoForm";
import RequestReasonSelector from "@/components/RequestReasonSelector";
import RoleSelector from "@/components/RoleSelector";
import SendButton from "@/components/SendButton";
import TextAreaWithCount from "@/components/TextAreaWithCount";
import React, { useState } from "react";
import { useTranslation } from "react-i18next";
import { toast, ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

function PropertyContents() {
  const { t, i18n } = useTranslation("mainInfo");
  const isArabic = i18n.language === "first_lang";

  const [formData, setFormData] = useState({
    fullName: "",
    phone: "",
    email: "",
    address: "",
    floor: "",
    property: "",
    whatsapp: false,
    propertyNumber: "",
    sectionNumber: "",
  });

  const [role, setRole] = useState<"owner" | "agent" | "tenant">("owner");
  const [agentFiles, setAgentFiles] = useState<File[]>([]);
  const [notes, setNotes] = useState("");
  const [errors, setErrors] = useState<Record<string, boolean>>({});

  // ⬇️ New lifted file states
  const [frontIdFile, setFrontIdFile] = useState<File | null>(null);
  const [backIdFile, setBackIdFile] = useState<File | null>(null);
  const [extractFile, setExtractFile] = useState<File | null>(null);
  const [comprehensivePropertyStatement, setComprehensivePropertyStatement] =
    useState<File[]>([]);
  const [selectedReason, setSelectedReason] = useState<
    "transfer" | "sale" | "license" | "other"
  >("transfer");
  const [saleFiles, setSaleFiles] = useState<File[]>([]);
  const [otherReason, setOtherReason] = useState("");

  const [status, setStatus] = useState<"no" | "yes">("no");
  const [permitType, setPermitType] = useState<
    "none" | "withoutHousing" | "withHousing"
  >("none");

  const [permitFilesNoHousing, setPermitFilesNoHousing] = useState<File[]>([]);
  const [permitFilesWithHousing, setPermitFilesWithHousing] = useState<File[]>(
    []
  );
  const [housingFiles, setHousingFiles] = useState<File[]>([]);
  const [selectedType, setSelectedType] = useState<"id" | "extract">("id");

  const [isSubmitting, setIsSubmitting] = useState(false);

  const validateForm = () => {
    const newErrors: Record<string, boolean> = {};

    // Basic fields
    if (!formData.fullName.trim()) newErrors.fullName = true;
    if (!formData.phone.trim()) newErrors.phone = true;
    if (!formData.floor.trim()) newErrors.floor = true;
    if (!formData.address.trim()) newErrors.address = true;
    if (!formData.property.trim()) newErrors.property = true;
    if (!formData.propertyNumber.trim()) newErrors.propertyNumber = true;

    // Role-specific
    if (role === "agent" && agentFiles.length === 0)
      newErrors.agentFiles = true;

    // DocumentUploader
    if (selectedType === "id") {
      if (!frontIdFile) newErrors.frontIdFile = true;
      if (!backIdFile) newErrors.backIdFile = true;
    } else if (selectedType === "extract") {
      if (!extractFile) newErrors.extractFile = true;
    }

    if (comprehensivePropertyStatement.length === 0) {
      newErrors.comprehensivePropertyStatement = true;
    }

    // RequestReasonSelector
    if (selectedReason === "sale" && saleFiles.length === 0) {
      newErrors.saleFiles = true;
    }
    if (selectedReason === "other" && !otherReason.trim()) {
      newErrors.otherReason = true;
    }

    // BuildingStatusSelector
    if (status === "yes") {
      if (
        permitType === "withoutHousing" &&
        permitFilesNoHousing.length === 0
      ) {
        newErrors.permitFilesNoHousing = true;
      }

      if (permitType === "withHousing") {
        if (permitFilesWithHousing.length === 0) {
          newErrors.permitFilesWithHousing = true;
        }
        if (housingFiles.length === 0) {
          newErrors.housingFiles = true;
        }
      }
    }

    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  const resetForm = () => {
    setFormData({
      fullName: "",
      phone: "",
      email: "",
      address: "",
      floor: "",
      property: "",
      whatsapp: false,
      propertyNumber: "",
      sectionNumber: "",
    });

    setRole("owner");
    setAgentFiles([]);
    setNotes("");
    setErrors({});

    setFrontIdFile(null);
    setBackIdFile(null);
    setExtractFile(null);
    setSelectedType("id");
    setComprehensivePropertyStatement([]);

    setSelectedReason("transfer");
    setSaleFiles([]);
    setOtherReason("");

    setStatus("no");
    setPermitType("none");
    setPermitFilesNoHousing([]);
    setPermitFilesWithHousing([]);
    setHousingFiles([]);
  };

  const handleSubmit = async () => {
    if (!validateForm()) {
      toast.error(t("toast.validationError"));
      return;
    }

    setIsSubmitting(true);

    const data = new FormData();
    Object.entries(formData).forEach(([key, val]) => {
      data.append(key, String(val));
    });

    data.append("role", role);
    data.append("notes", notes);
    data.append("selectedType", selectedType);
    data.append("selectedReason", selectedReason);
    if (selectedReason === "other") {
      data.append("otherReason", otherReason);
    }

    data.append("status", status);
    data.append("permitType", permitType);

    // Files
    if (frontIdFile) data.append("frontId", frontIdFile);
    if (backIdFile) data.append("backId", backIdFile);
    if (extractFile) data.append("extract", extractFile);
    comprehensivePropertyStatement.forEach((file) =>
      data.append("comprehensivePropertyStatement", file)
    );
    agentFiles.forEach((file) => data.append("agentFiles", file));
    saleFiles.forEach((file) => data.append("saleFiles", file));
    permitFilesNoHousing.forEach((file) =>
      data.append("permitFilesNoHousing", file)
    );
    permitFilesWithHousing.forEach((file) =>
      data.append("permitFilesWithHousing", file)
    );
    housingFiles.forEach((file) => data.append("housingFiles", file));

    try {
      const res = await fetch(
        "/api/property-certificate",
        {
          method: "POST",
          body: data,
        }
      );

      const result = await res.json();
      if (result.success) {
        toast.success(t("toast.submitSuccess"));
        resetForm();
      } else {
        toast.error(t("toast.serverError"));
      }
    } catch (error) {
      console.error("Submission error:", error);
      toast.error(t("toast.serverError"));
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <div className="bg-lightGray min-h-screen">
      <div className="w-11/12 mx-auto py-10">
        <BackToRequests />

        <p className="text-4xl xl:text-[40px] font-bold text-darkOlive pb-8">
          {t("realEstate.title5")}
        </p>

        <div className="gap-4 flex flex-col md:text-2xl text-xl text-coralRed font-semibold max-w-xl pb-8">
          <p>{t("clearanceNote1")}</p>
          <p>{t("clearanceNote2")}</p>
        </div>

        <div className="flex flex-col border-b border-gray-400">
          <RoleSelector
            role={role}
            setRole={setRole}
            agentFiles={agentFiles}
            setAgentFiles={setAgentFiles}
            errors={errors}
          />
          <InfoForm
            formData={{
              fullName: formData.fullName,
              phone: formData.phone,
              email: formData.email,
              address: formData.address,
              floor: formData.floor,
              property: formData.property,
              whatsapp: formData.whatsapp,
            }}
            setFormData={(partialData) =>
              setFormData((prev) => ({
                ...prev,
                ...(typeof partialData === "function"
                  ? partialData(prev)
                  : partialData),
              }))
            }
            errors={errors}
          />

          <ClearanceRequestSection
            formData={{
              propertyNumber: formData.propertyNumber,
              sectionNumber: formData.sectionNumber,
            }}
            setFormData={(partialData) =>
              setFormData((prev) => ({
                ...prev,
                ...(typeof partialData === "function"
                  ? partialData(prev)
                  : partialData),
              }))
            }
            errors={errors}
          />

          <BuildingStatusSelector
            status={status}
            setStatus={setStatus}
            permitType={permitType}
            setPermitType={setPermitType}
            permitFilesNoHousing={permitFilesNoHousing}
            setPermitFilesNoHousing={setPermitFilesNoHousing}
            permitFilesWithHousing={permitFilesWithHousing}
            setPermitFilesWithHousing={setPermitFilesWithHousing}
            housingFiles={housingFiles}
            setHousingFiles={setHousingFiles}
            errors={errors}
          />
          <RequestReasonSelector
            selectedReason={selectedReason}
            setSelectedReason={setSelectedReason}
            saleFiles={saleFiles}
            setSaleFiles={setSaleFiles}
            otherReason={otherReason}
            setOtherReason={setOtherReason}
            errors={errors}
          />
          <DocumentUploader
            selectedType={selectedType}
            setSelectedType={setSelectedType}
            frontIdFile={frontIdFile}
            setFrontIdFile={setFrontIdFile}
            backIdFile={backIdFile}
            setBackIdFile={setBackIdFile}
            extractFile={extractFile}
            setExtractFile={setExtractFile}
            comprehensivePropertyStatement={comprehensivePropertyStatement}
            setComprehensivePropertyStatement={
              setComprehensivePropertyStatement
            }
            errors={errors}
          />
        </div>
        <TextAreaWithCount
          label={t("notes")}
          maxLength={250}
          onChange={setNotes}
          value={notes}
        />
        <SendButton
          text={t("send")}
          onClick={handleSubmit}
          loading={isSubmitting}
        />
        <ClearanceInfoSection />
      </div>
      <ToastContainer
        position={isArabic ? "bottom-left" : "bottom-right"}
        autoClose={3000}
        hideProgressBar={false}
      />
    </div>
  );
}

export default PropertyContents;
