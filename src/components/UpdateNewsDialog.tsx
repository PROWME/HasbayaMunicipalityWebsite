...
export default function UpdateNewsDialog({
  ...
  const [titleAr, setTitleAr] = useState(newsItem.title_first_lang);
  const [titleEn, setTitleEn] = useState(newsItem.title_english);
  const [descAr, setDescAr] = useState(newsItem.description_first_lang);
  const [descEn, setDescEn] = useState(newsItem.description_english);
  const [images, setImages] = useState<File[]>([]);
  const [oldImages, setOldImages] = useState(newsItem.images);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(
        `/api/news/updateNews/${newsItem.id}`,
        formData,
        {
          headers: {
